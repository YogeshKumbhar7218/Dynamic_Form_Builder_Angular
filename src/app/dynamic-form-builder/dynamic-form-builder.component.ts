...
export class DynamicFormBuilderComponent {
    ...
    constructor(public formBuilder: FormBuilder, private formlyConverterService: FormlyConverterService) {
        // this.createForm();
        // this.convertFromJson(this.json);
    ...
    formPreview() {
        ...
        let jsonString = JSON.stringify(this.fields)
        console.log("Fields", jsonString)

        console.log("Fields", this.fields)
    }

    // json = [
    //   { "key": "", "type": "input", "templateOptions": { "attributes": { "spellcheck": "" } } },
    //   { "key": "", "type": "textarea", "templateOptions": {} },
    //   { "key": "", "type": "checkbox", "templateOptions": { "hideLabel": false, "hideRequiredMarker": false, "indeterminate": false, "readonly": false } },
    //   { "key": "", "type": "radio", "templateOptions": { "hideLabel": false, "hideRequiredMarker": false, "readonly": false, "options": [] } }
    // ]

    json = ''

    loadFormInCanvas(json: string) {
        this.canvasItems = [];
        let fieldsArray = JSON.parse(json);
        fieldsArray.forEach(element => {
            this.addItem(element.type);
        });
        this.canvasItems.forEach((element, i) => {
            this.formlyConverterService
                .assignJsonValuesToForm(fieldsArray[i], element.formGroup)
        })
    }
}